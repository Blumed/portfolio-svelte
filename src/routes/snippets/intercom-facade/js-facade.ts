export default `(function(){
    ...
    const companynameIntercomFacade = () => {
        //Facade button
        const buttonFacade = document.createElement('button');
        buttonFacade.type = 'button';
        buttonFacade.className = config.facadeContainerClass;
        buttonFacade.setAttribute('onclick', 'window.companynameInitIntercomRemoveFacade();setInterval(Intercom(\'show\'), 500);');
        ...
        const facadeInnerWrapper = document.createElement('div');
        ...
        const facadeIntercomLogo = document.createElement('img');
        ...
        facadeIntercomLogo.setAttribute('alt', '');
        facadeInnerWrapper.appendChild(facadeIntercomLogo);

        //Facade button hover style
        const facadeHoverStyle = document.createElement('style');
    ...
    // Fire check urls on initial page load
    window.checkUrls();

    if(config.siteIsSPA) {
        // If you are using an SPA mutation observer will fire when the document changes
        const observer = new MutationObserver(function(previousUrl = '') {
    ...
    // Bypass is empty so facade will load no matter what. If you are using an SPA this will persist and only needs to load once
